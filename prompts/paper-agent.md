...
- If the paper is a method or system paper, use `visualization.type = "architecture"` and build a true directed graph rather than a linear chain.
...
- Prefer symbolic tensor shapes everywhere. Put the most useful shape information on `edges.shape`, not only on the node.
